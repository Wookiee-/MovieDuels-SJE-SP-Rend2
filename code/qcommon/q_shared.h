...
constexpr auto CLIENT_WINDOW_TITLE = "MovieDuels-SP (SP)";
constexpr auto CLIENT_CONSOLE_TITLE = "MovieDuels-SP Console (SP)";
...
//
constexpr auto MAX_MAP_AREA_BYTES = 32; // bit vector of area visibility;
...
#define GCJ_VS_2013
#define NEW_FEEDER
#define NEW_FEEDER_V1
#define NEW_FEEDER_V2
#define NEW_FEEDER_V3
#define NEW_FEEDER_V4
#define NEW_FEEDER_V5
#define NEW_FEEDER_V6
#define NEW_FEEDER_V7
#define NEW_FEEDER_V8
#define NEW_FEEDER_V9
...
constexpr auto CVAR_SAVEGAME = 256; // store this in the save game;
...
constexpr auto SNAPFLAG_RATE_DELAYED = 1;
constexpr auto SNAPFLAG_NOT_ACTIVE = 2; // snapshot used during connection and for zombies;
constexpr auto SNAPFLAG_SERVERCOUNT = 4; // toggled every map_restart so transitions can be detected;
...
#else
constexpr auto MAX_WORLD_FX = 128; // was 16 // was 4;
#endif // JK2_MODE
...
using saberTrail_t = struct
{
	...
	int lastTime; // time a saber segment was last stored
...
using saberInfo_t = struct
{
	...
	int combatstanceAnim; //-1 - anim to use when hit "combatstance"
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(combatstanceAnim);
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(combatstanceAnim);
...
public:
	...
	qboolean SaberStaff()
	{
		return static_cast<qboolean>(saber[0].type == SABER_STAFF || saber[0].type == SABER_STAFF_UNSTABLE || saber[0].
			type ==
			SABER_STAFF_SFX || saber[0].type == SABER_STAFF_THIN || dualSabers && saber[1].type == SABER_STAFF);
	};
	...
	int projectToggleTime;
	qboolean flamethrowerOn;
	qboolean IsSprinting;
	qboolean IsBlockingLightning;

	int sprintFuel;
	int sprintkDebRecharge;
	int sprintDebReduce;
	qboolean Isprojecting;
	...
	int destructlaststartTime;

	int projecttimeplayer;
	int projectstartTime;
	int projectlaststartTime;
		...
		ojk::SavedGameHelper& saved_game) const
	{
		...
		saved_game.write<int32_t>(projectToggleTime);
		saved_game.write<int32_t>(flamethrowerOn);
		saved_game.write<int32_t>(IsSprinting);
		saved_game.write<int32_t>(IsBlockingLightning);
		...
		saved_game.write<int32_t>(destructlaststartTime);

		saved_game.write<int32_t>(projecttimeplayer);
		saved_game.write<int32_t>(projectstartTime);
		saved_game.write<int32_t>(projectlaststartTime);
		...
		ojk::SavedGameHelper& saved_game)
	{
		...
		saved_game.read<int32_t>(projectToggleTime);
		saved_game.read<int32_t>(flamethrowerOn);
		saved_game.read<int32_t>(IsSprinting);
		saved_game.read<int32_t>(IsBlockingLightning);
		...
		saved_game.read<int32_t>(destructlaststartTime);

		saved_game.read<int32_t>(projecttimeplayer);
		saved_game.read<int32_t>(projectstartTime);
		saved_game.read<int32_t>(projectlaststartTime);
...
constexpr auto BUTTON_BLOCKING = 8;
...
constexpr auto BUTTON_LIGHTNING_STRIKE = 2048; //+button11;
constexpr auto BUTTON_GRAPPLE = 4096; //+button12 // Unused;
...
constexpr auto BUTTON_PROJECTION = 65536; //+button16;
