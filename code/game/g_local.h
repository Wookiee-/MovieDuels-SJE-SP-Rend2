...
#define FL_NO_SABER_RETURN		0x00008000	//prevents NPC hilts from being auto-returned to their hand after a short time.
...
#define FL_VADERBREATH		    0x00800000	// VADERBREATH
#define FL_VADERBREATHDAMAGED   0x01000000	// VADERBREATHDAMAGED
#define FL_FORCEMODE            0x02000000	// unlimitedpower
...
public:
	...
	float addLight; //additional light- makes it more noticeable, even in darkness
	int ID;
...
void G_SetBoltSurfaceRemoval(int entNum, int modelIndex, int bolt_index, int surface_index = -1,
	float duration = 5000);
/*
Ghoul2 Insert End
*/
...
// g_client.c
//
char* ClientConnect(int clientNum, qboolean first_time, SavedGameJustLoaded_e e_saved_game_just_loaded);
void client_userinfo_changed(int clientNum);
void ClientDisconnect(int clientNum);
void ClientBegin(int clientNum, const usercmd_t* cmd, SavedGameJustLoaded_e e_saved_game_just_loaded);
