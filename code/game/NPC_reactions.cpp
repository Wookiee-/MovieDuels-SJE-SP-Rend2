	...
	const int voice_event = -1)
{
	...
	if (Q_flrand(0.0f, 1.0f) < pain_chance)
	{
		...
		else if (self->client->ps.eFlags & EF_FORCE_GRASPED)
		{
	...
	const int hit_loc)
{
	...
	if (self->client && (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_MANDALORIAN ||
...
void NPC_Use(gentity_t* self, gentity_t* other, gentity_t* activator)
{
	...
	if (self->client && self->NPC)
	{
		...
		else if (jedi_waiting_ambush(NPC))
		{
			jedi_ambush(NPC);
		}
