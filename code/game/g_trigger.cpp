	...
	if (owner_trigger)
	{
		...
		for (int axis = 0; axis < 3; axis++)
		{
			if (axis == capAxis)
			{
				if (Q_irand(0, 1))
				{
					startSpot[axis] = owner_trigger->mins[axis];
					endSpot[axis] = owner_trigger->maxs[axis];
				}
				else
				{
					startSpot[axis] = owner_trigger->maxs[axis];
					endSpot[axis] = owner_trigger->mins[axis];
				}
			}
			else
			{
				startSpot[axis] = owner_trigger->mins[axis] + flrand(0, 1.0f) * (owner_trigger->maxs[axis] -
					owner_trigger
					->mins[axis]);
				endSpot[axis] = owner_trigger->mins[axis] + flrand(0, 1.0f) * (owner_trigger->maxs[axis] - owner_trigger
					->
					mins[axis]);
			}
		}
