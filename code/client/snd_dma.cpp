...
void S_Init()
{
	...
	Cmd_AddCommand("totgmapmusic", S_totgmapmusic_f);
...
channel_t* S_OpenALPickChannel(const int entnum, const int entchannel)
{
	...
	if (!foundChan)
	{
		...
		if (ch->fixed_origin)
		{
			...
			longest_dist = ((listener_pos[0] - source_pos[0]) * (listener_pos[0] - source_pos[0])) +
...
static int S_CheckAmplitude(channel_t* ch, const int s_oldpaintedtime)
{
		...
		//(ch->startSample == START_SAMPLE_IMMEDIATE)//!s_entityWavVol_back[ch->entnum]
	{
		...
		short* current_pos_s = ch->thesfx->pSoundData;
		...
		for (int i = 1; i < MAX_CHANNELS; i++, ch++)
		{
			...
			if (ch->thesfx->pMP3StreamHeader)
			{
				...
				if (i >= NUM_STREAMING_BUFFERS)
					n_buffers_to_add = NUM_STREAMING_BUFFERS;
				else
					n_buffers_to_add = i + 1;

				// Make sure queue is empty first
				alSourcei(s_channels[source].alSource, AL_BUFFER, NULL);

				for (i = 0; i < n_buffers_to_add; i++)
				{
...
byte* SND_malloc(const int iSize, sfx_t* sfx)
{
	...
	{
		while ((Z_MemSize(TAG_SND_RAWDATA) + Z_MemSize(TAG_SND_MP3STREAMHDR)) > ((-s_soundpoolmegs->integer) * 1024 *
			1024))
		{
...
static void UnloadEALFile()
{
	...
	HRESULT hr = s_lpEAXManager->FreeDataSet(0);
	s_bEALFileLoaded = false;
...
static void UpdateEAXListener()
{
	...
	if ((s_lLastEnvUpdate + ENV_UPDATE_RATE) < lCurTime)
	{
		...
		{
			if ((s_FXSlotInfo[i].lEnvID != -1) && (s_FXSlotInfo[i].lEnvID != ReverbData[0].lEnvID) && (s_FXSlotInfo[i].
				lEnvID != ReverbData[1].lEnvID)
